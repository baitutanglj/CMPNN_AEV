...
def select(
    ...
    resselection = system.select_atoms(
        ...
        # f"(resname LIG)"
        # f"(byres (around {distance} (resname LIG)))"
    )
...
def pad_collate(
...
) -> Tuple[torch.Tensor, np.ndarray, torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
    """
    ...
    maskLIG, ids, labels, species_and_coordinates = zip(*batch)
    ...
    pad_maskLIG = nn.utils.rnn.pad_sequence(
        maskLIG, batch_first=True, padding_value=0
    )

    return pad_maskLIG, np.array(ids), torch.tensor(labels), (pad_species, pad_coordinates)
...
def chemap(
    ...
    for idx in range(n):
        for to_element, from_elements in cmapZ.items():
            mask = np.isin(atomicnums[idx], from_elements, invert=True)
            atomicnums[idx][mask] = to_element
    return 0
...
class Data(data.Dataset):
    ...
    def __getitem__(
        self, idx: int
    ) -> Tuple[list, str, float, Tuple[torch.Tensor, torch.Tensor]]:
        """
...
class PDBData(Data):
    ...
    def __init__(
        ...
        removeHs: bool = False,
        no_contain_header: bool = False
    ):

        super().__init__()

        self._load(fname, distance, datapaths, cmap, desc, removeHs, no_contain_header)
    ...
    def _load(
        ...
        self.resselection = []
        self.maskLIG = []
        self.contain_header = not no_contain_header
        total = len(open(fname, 'r').read().splitlines())
        with open(fname, "r") as f:
            if self.contain_header:
                next(f)
                total = total - 1
            for line in tqdm.tqdm(f.read().splitlines(), total=total, desc=desc):
                label, recfile, ligfile, smiles = line.split(',')

                self.ids.append(os.path.dirname(recfile))

                self.labels.append(float(label))

                resselection, els, coords = load_pdbs_and_select(
                ...
                self.resselection.append(resselection)
                maskLIG = resselection.resnames =='LIG'
                self.maskLIG.append(torch.from_numpy(maskLIG))
