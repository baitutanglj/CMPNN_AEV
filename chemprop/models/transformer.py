...
class Embeddings(nn.Module):
    ...
    """
    def __init__(self, d_model, vocab:int=None):
        super(Embeddings, self).__init__()
        # self.lut = nn.Embedding(vocab, d_model)
        self.vocab = vocab
        if self.vocab:
            self.ft = nn.Linear(self.vocab, d_model)
        self.d_model = d_model

    def forward(self, x):
        if self.vocab:
            assert x.size(-1) == self.vocab, 'x.size(-1):{} != self.vocab:{}'.format(x.size(-1),self.vocab)
            x = self.ft(x)
...
def seq_padding(seq, pad=0):
    ...
    seq_mask = seq_mask.bool().unsqueeze(-2).to(seq.device)
    return seq, seq_mask
...
class Transformer(nn.Module):
    ...
    def forward(self, src, tgt):
        ...
        src, src_mask = seq_padding(src)
        src = src.cuda(tgt[0].get_device())
        src_mask = src_mask.cuda(tgt[0].get_device())
        tgt, tgt_mask = seq_padding(tgt)
...
""" 对模型进行初始化"""
def make_model(d_model:int=512,
               nhead:int=8,
               d_ff:int=1024,
               layers:int=6,
               dropout:float=0.1,
    ...
    c = copy.deepcopy
    # 实例化Attention对象
    attn = MultiHeaderAttention(nhead, d_model)
    # 实例化FeedForward对象
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src, tgt):
        ...
        seq_lens = [len(x) for x in src]
        max_len = max(seq_lens)

        ##padding pad=0
        pad = 0
        src = nn.utils.rnn.pad_sequence(
            src, batch_first=True, padding_value=pad
        )

        # #mask
        batch_size = src.shape[0]
        src_mask = torch.zeros((batch_size,max_len))
        for i in range(batch_size):
            src_mask[i,:seq_lens[i]] = 1
        src_mask = src_mask.bool().unsqueeze(-2).to(src.device)
...
def make_encoder(d_model:int=512,
                        nhead:int=8,
                        d_ff:int=1024,
                        layers:int=6,
                        dropout:float=0.1,
